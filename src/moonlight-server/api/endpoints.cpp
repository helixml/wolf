...
void UnixSocketServer::endpoint_AddApp(const HTTPRequest &req, std::shared_ptr<UnixSocket> socket) {
  ...
  if (app) {
    state_->app_state->config->apps->update([app = app.value(), this](auto &apps) {
      ...
      std::string default_video_producer_buffer_caps = use_zero_copy ? "video/x-raw(memory:DMABuf)" : "video/x-raw";

      // When pipelines are not specified, Wolf will use its internal defaults
      // These defaults automatically respect the WOLF_USE_ZERO_COPY setting
      // DO NOT hardcode pipelines here - let Wolf's streaming.cpp handle it
      ...
      return apps.push_back(events::App{
          ...
          .h264_gst_pipeline = app.h264_gst_pipeline.value_or(""),  // Let Wolf use defaults
          .hevc_gst_pipeline = app.hevc_gst_pipeline.value_or(""),  // Let Wolf use defaults
          .av1_gst_pipeline = app.av1_gst_pipeline.value_or(""),    // Let Wolf use defaults
          .render_node = app.render_node.value_or("/dev/dri/renderD128"),  // Use system default
          .opus_gst_pipeline = app.opus_gst_pipeline.value_or(""),  // Let Wolf use defaults
